...
export default function SuperAdminStaffContent({ initialStaff }: { initialStaff: StaffMember[] }) {
  ...
  const applyToggle = async () => {
    ...
    const nextActive = confirmTarget.status !== 'Active'
    setToggling(true)
    const outcome = await setStaffActiveAction({ id: confirmTarget.id, active: nextActive })
    setToggling(false)
    if (outcome.ok) {
      setStaff((prev) => prev.map((s) => s.id === confirmTarget.id ? { ...s, status: nextActive ? 'Active' : 'Inactive' } : s))
    }
          ...
          <div className="relative w-full sm:max-w-sm bg-white rounded-3xl shadow-2xl z-10 overflow-hidden">
            ...
            <div className="px-6 py-5">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={applyToggle} disabled={toggling} className={`flex-1 py-3 text-[13px] font-semibold rounded-xl text-white transition-colors disabled:opacity-60 ${confirmTarget.status === 'Active' ? 'bg-danger hover:bg-danger/90' : 'bg-success hover:bg-success/90'}`}>
                  {toggling ? 'Saving…' : confirmTarget.status === 'Active' ? 'Deactivate' : 'Reactivate'}
                </button>
              </div>
            </div>
          </div>
