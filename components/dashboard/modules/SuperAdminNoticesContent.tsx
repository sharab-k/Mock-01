...
import { ArrowLeft, Plus, X, Megaphone, AlertCircle } from 'lucide-react'
import { CATEGORY_STYLE, type Notice, type NoticeCategory, type NoticeAudience } from '@/lib/notices/types'
import { createNoticeAction, updateNoticeAction, setNoticePublishedAction } from '@/lib/actions/notices'
...
export default function SuperAdminNoticesContent({ initialNotices }: { initialNotices: Notice[] }) {
  ...
  const openEdit = (n: Notice) => { setEditingId(n.id); setDraft({ title: n.title, body: n.body, category: n.category, audience: n.audience }); setError(''); setModalOpen(true) }
  ...
  const save = async () => {
    ...
    if (editingId) {
      const outcome = await updateNoticeAction({ id: editingId, ...draft })
      setSaving(false)
      if (!outcome.ok) { setError(outcome.error); return }
      setNotices((prev) => prev.map((n) => n.id === editingId ? { ...n, ...draft } : n))
    } else {
      const outcome = await createNoticeAction(draft)
      setSaving(false)
      if (!outcome.ok) { setError(outcome.error); return }
      // Use the real id back from the insert — an editable/toggleable row
      // needs a real UUID, not a placeholder, in case the admin acts on it
      // again before the next page load.
      setNotices((prev) => [{
        id: outcome.id,
        ...draft,
        published_at: new Date(outcome.publishedAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }),
        published: true,
      }, ...prev])
    }
    setModalOpen(false)
  ...
  const togglePublish = async (n: Notice) => {
    ...
    setNotices((prev) => prev.map((x) => x.id === n.id ? { ...x, published: nextPublished } : x))
    const outcome = await setNoticePublishedAction({ id: n.id, published: nextPublished })
    if (!outcome.ok) setNotices((prev) => prev.map((x) => x.id === n.id ? { ...x, published: n.published } : x))
  }
          ...
          {notices.map((n) => (
            <div key={n.id} className="flex items-start gap-4 px-5 py-4">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button onClick={() => togglePublish(n)} className="text-[11.5px] font-medium text-neutral-500 hover:text-neutral-800 px-2.5 py-1.5 rounded-lg hover:bg-neutral-100 transition-colors">
          ...
          <div className="relative w-full sm:max-w-md bg-white rounded-3xl shadow-2xl z-10 overflow-hidden">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                <button onClick={save} disabled={!draft.title.trim() || saving} className="flex-1 py-3 text-[13px] font-semibold rounded-xl bg-ink-700 text-white hover:bg-ink-800 transition-colors disabled:opacity-50">
