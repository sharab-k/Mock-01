...
export default function AttendanceStudentsListContent({ students }: { students: GroupedStudent[] }) {
  ...
  return (
    <>
      <div>
        ...
        <p className="text-[13px] text-neutral-500 mt-0.5">Grouped by class — {students.length} students</p>
      </div>
                  ...
                  {sec.students.map((s) => (
                    <div key={s.id} className="flex items-center gap-2.5">
                      ...
                      {s.attendancePct === null ? (
                        <span className="text-[10.5px] text-neutral-400 italic shrink-0">No data</span>
                      ) : (
                        <span className={`text-[11px] font-mono font-semibold shrink-0 ${s.attendancePct >= 90 ? 'text-success' : s.attendancePct >= 80 ? 'text-warning' : 'text-danger'}`}>{s.attendancePct}%</span>
                      )}
                    </div>
                  ))}
                </div>
