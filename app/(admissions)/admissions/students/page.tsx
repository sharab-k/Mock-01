...
import { STUDENTS as INITIAL_STUDENTS, GRADES, INITIALS, type Student } from '@/lib/mock/students'
...
export default function AdmissionsStudentsPage() {
      ...
      <div>
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="text-[13px] text-neutral-500 mt-0.5">{filtered.length} of {students.length} sample records</p>
          </div>
            ...
            <tbody className="divide-y divide-neutral-100">
              {filtered.map((s) => (
                <tr key={s.id} className={`hover:bg-neutral-50 transition-colors ${s.status === 'Inactive' ? 'opacity-60' : ''}`}>
                  ...
                  <td className="px-3 py-3.5 text-neutral-500 hidden sm:table-cell">{s.parent_name}</td>
