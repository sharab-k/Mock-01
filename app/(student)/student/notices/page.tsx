...
import { NOTICES, CATEGORY_STYLE } from '@/lib/mock/notices'

export default function StudentNoticesPage() {
  const [expanded, setExpanded] = useState<string | null>(null)
  const published = NOTICES.filter((n) => n.published && (n.audience === 'All' || n.audience === 'Students'))
  ...
  return (
    <>
      <div>
        ...
        <p className="text-[13px] text-neutral-500 mt-0.5">{published.length} notices for you</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-neutral-200 shadow-1 overflow-hidden">
        <div className="divide-y divide-neutral-100">
          {published.map((n) => (
            <div key={n.id}>
              <button onClick={() => setExpanded(expanded === n.id ? null : n.id)} className="w-full flex items-start gap-4 px-5 py-4 hover:bg-neutral-50 transition-colors text-left">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className="text-[11px] font-mono text-neutral-400">{n.published_at}</span>
                </div>
