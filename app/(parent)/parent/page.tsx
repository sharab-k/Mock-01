...
import { CalendarCheck, BookOpen, TrendingUp, ShieldCheck, CheckCircle2, Clock3, XCircle } from 'lucide-react'

// ── Data ──────────────────────────────────────────────────────────────────────
const CHILDREN = [
  { name: 'Ahmed Hassan', roll: 'JE-2026-001', programme: 'Matriculation', class: 'X-A',   initials: 'AH', grade: 'B+' },
  { name: 'Sara Hassan',  roll: 'JE-2026-088', programme: 'Middle School', class: 'VII-B', initials: 'SH', grade: 'B'  },
]
  ...
  return (
    <>
        ...
        <div className="flex">
          {CHILDREN.map((c, i) => (
            <div
              key={c.roll}
              className={[
                'flex items-center gap-4 flex-1 px-6 py-4 cursor-pointer transition-all border-b-2',
            ...
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-[13px] shrink-0 transition-colors ${
                i === 0 ? 'bg-ink-700 text-white' : 'bg-neutral-100 text-neutral-500'
              }`}>
                {c.initials}
              </div>
              <div className="min-w-0">
                <span className={`block text-[14px] font-semibold truncate ${i === 0 ? 'text-neutral-900' : 'text-neutral-500'}`}>{c.name}</span>
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  <span className="text-neutral-300">·</span>
                  <span className={`text-[11px] font-semibold ${i === 0 ? 'text-ink-600' : 'text-neutral-400'}`}>Grade {c.grade}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="bg-white rounded-2xl border border-neutral-200 shadow-1 overflow-hidden">
        ...
        {/* Legend */}
        <div className="flex items-center gap-5 px-6 pt-4 pb-2">
        ...
        <div className="mx-6 mb-6 p-4 bg-ink-50 rounded-xl border border-ink-100/60">
          <p className="text-[12px] text-ink-600 leading-relaxed">
            ...
            {' '}Contact <span className="font-mono">admissions@jeacademy.edu.pk</span> for credential issues.
          </p>
        </div>
      </div>
    </>
