...
// Shared by the parent dashboard's sibling switcher (Phase 5) and every
// student-view screen (Phase 6, reached through the parent's own session for
// one specific linked child) so the two surfaces can never quietly disagree
// on how "attendance %" or "average score" is computed. Runs on the caller's
// own RLS-scoped client — parent_read_linked_children_* policies are what
// actually authorize this read, not application logic.
