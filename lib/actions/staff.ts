...
export async function createStaffAction(input: z.infer<typeof CreateStaffSchema>): Promise<CreateStaffResult> {
  const parsed = CreateStaffSchema.safeParse(input)
  if (!parsed.success) return { ok: false, error: 'Invalid administrator details.' }

  const { userId, authorized } = await requireSuperAdminCaller()
